...
func CreateSplitRegionOperator(desc string, region *core.RegionInfo, kind OpKind, policy pdpb.CheckPolicy, keys [][]byte) (*Operator, error) {
	...
	op := NewOperator(desc, brief, region.GetID(), region.GetRegionEpoch(), kind, int64(region.GetApproximateSize()), step)
...
func CreateMergeRegionOperator(desc string, ci sche.SharedCluster, source *core.RegionInfo, target *core.RegionInfo, kind OpKind) ([]*Operator, error) {
	...
	op1 := NewOperator(desc, brief, source.GetID(), source.GetRegionEpoch(), kind, int64(source.GetApproximateSize()), steps...)
	op2 := NewOperator(desc, brief, target.GetID(), target.GetRegionEpoch(), kind, int64(target.GetApproximateSize()), MergeRegion{
		FromRegion: source.GetMeta(),
		ToRegion:   target.GetMeta(),
		IsPassive:  true,
	})
...
func CreateLeaveJointStateOperator(desc string, ci sche.SharedCluster, origin *core.RegionInfo) (*Operator, error) {
	...
	return NewOperator(b.desc, brief, b.regionID, b.regionEpoch, kind, int64(origin.GetApproximateSize()), b.steps...), nil
}
